...
public class Client extends JFrame{
	...
	private void password(){
		...
		try {
			...
			String url = "jdbc:mysql://106.13.180.138/java_test?serverTimezone=UTC&useSSL=false&useUnicode=true&characterEncoding=utf8";
			String user = "root";
	 ...
	 */
	private void zhuce() {
		JDialog zhuceView = new JDialog(this,true);
		zhuceView.setTitle("用户注册");
		zhuceView.setSize(200, 300);
		zhuceView.setLayout(new GridLayout(9,2));
		JLabel usernameLabel=new JLabel("姓名:*");
		JLabel passwordLabel=new JLabel("密码:*");
		JLabel conformPasswordLabel=new JLabel("确认密码:*");
		JLabel sexLabel=new JLabel("性别:");
		JLabel phoneLabel=new JLabel("手机号:");
		JLabel qqLabel=new JLabel("qq号:");
		JLabel photoLabel=new JLabel("头像地址:");
		JLabel stayPlace=new JLabel("常住地址:");
		JTextField usernameField=new JTextField(10);
		JTextField passwordField=new JTextField(10);
		JTextField conformPasswordField=new JTextField(10);
		JTextField sexField=new JTextField(10);
		JTextField phoneField=new JTextField(10);
		JTextField qqField=new JTextField(10);
		JTextField photoField=new JTextField(10);
		JTextField stayPlaceField=new JTextField(10);
		JButton zhuce = new JButton("注册");
		JButton close = new JButton("取消");
		zhuceView.add(usernameLabel);
		zhuceView.add(usernameField);
		zhuceView.add(passwordLabel);
		zhuceView.add(passwordField);
		zhuceView.add(conformPasswordLabel);
		zhuceView.add(conformPasswordField);
		zhuceView.add(sexLabel);
		zhuceView.add(sexField);
		zhuceView.add(phoneLabel);
		zhuceView.add(phoneField);
		zhuceView.add(qqLabel);
		zhuceView.add(qqField);
		zhuceView.add(photoLabel);
		zhuceView.add(photoField);
		zhuceView.add(stayPlace);
		zhuceView.add(stayPlaceField);
		zhuceView.add(close);
		zhuceView.add(zhuce);
		zhuceView.setLocationRelativeTo(null);		
		close.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				zhuceView.setVisible(false);
			}
		});
		...
		zhuce.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				int Sex = 0;
				if(sexField.getText() == "男") {
					Sex=1;
				}
				if(usernameField.getText()==null||passwordField.getText()==null||conformPasswordField.getText()==null) {
					JOptionPane.showMessageDialog(null, "带*号的为必填项，请重试");
				}
				if(passwordField.getText().equals(conformPasswordField.getText())) {
					try {
						...
						String passwString="summer";
						Connection connection=DriverManager.getConnection(url,userString,passwString);
						Statement statement=connection.createStatement();
						String sqlString="insert into user(username,password)values('"+usernameField.getText()+"','"+passwordField.getText()+"')";
					...
					System.out.println(conformPasswordField.getText());
	...
	private void connect() {
		try {
			//指定连接服务器与端口
			//查找端口命令：sudo lsof -i:9999
			//根据PID号进行杀端口:kill -9 PID号
